...
class OTPDialog extends StatelessWidget {
  const OTPDialog({Key? key, required this.request, required this.completer}) :super(key: key);
    ...
    return Card(
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(
            width: 1,
            color: const Color.fromRGBO(42, 42, 42, 0.04),
          ),
        ),
        child: Column(
  ...
  if(customData is OTPDialogStatus){
    switch(customData){
      case OTPDialogStatus.success:
        return Icon(Icons.rocket_launch_sharp,
        color: Colors.green,
        size: 100.r,);
