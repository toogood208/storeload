import 'package:flutter/material.dart';

Color kBackgroundColor = const Color(0XFF001B2E);
 const kWhiteColor =  Color(0XFFFFFFFF);
const kTextColor = Color(0XFF2A2A2A);
const kTextColor40 = Color(0XFF666666);
const kTextColor30 = Color(0XFF4D4D4D);
const kTextColor50 = Color(0XFF808080);
const kTextColor60 = Color(0XFF999999);
const kTextColor80 = Color(0XFFCCCCCC);
const kTextColor20 = Color(0XFF333333);
const kInactiveBottomNavigationColor = Color(0XFFD9D9D9);
const kSecondOrangeColor = Color(0XFFFF9447);
const kErrorTextColor = Color(0XFFD41F1F);
const kSuccessTextColor = Colors.green;
const kTextFieldFocusedColor = Color.fromRGBO(0, 27, 46, 0.08);
const kProductDetailActiveColor=Color(0XFFFF7F11);
const kProductDetailInActiveColor=Color(0XFFE6E6E6);